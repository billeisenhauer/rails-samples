...
# NOTE: Sitemap creator taken from:
#       http://www.webficient.com/2008/09/06/google-sitemap-ruby-on-rails
#
# Minor formatting changes made to the original. 
class Crawler
  ...
  def initialize(starting_url, credentials = nil, quiet_mode = false, 
                 sitemap = false, debug = false)
    @bad_pages = []  
  ...
  def extract_and_call_urls(url)            
    ...
    # For any content types we may have missed above, exit if content type is 
    # not html
